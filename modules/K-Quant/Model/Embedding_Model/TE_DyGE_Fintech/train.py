...
# Save id to stock code
id2stock = {}
for node in new_G.nodes(data=True):
    id2stock[node[1]['code']] = node[0]

a = num_time_steps - 2
json_str = json.dumps(id2stock)
with open('stock2id'+str(a)+ '.json', 'w') as json_file:
    json_file.write(json_str)


# for nodeid in graphs[num_time_steps - 1].nodes(data=True):
#     # if nodeid >= len(graphs[num_time_steps - 1]):
#     #     print('overindex',nodeid)
#     #     continue 
#     new_G.add_nodes(nodeid)

# new_G.add_nodes_from(graphs[num_time_steps - 1].nodes(data=True))
# print('length of new G',len(new_G.nodes()))
...
# for x in feats :
#     if x.shape[0] <= feats[num_time_steps - 1].shape[0]:
#         print('in',x.shape[0],feats[num_time_steps - 1].shape[0])
...
print('adj shape',adjs[num_time_steps - 1].shape[0]) 
print("feats_2",feats)
num_features = feats[0].shape[1]
...
np.savez(SAVE_DIR + '/{}_embs_{}_{}.npz'.format(FLAGS.model, FLAGS.dataset, num_time_steps), data=emb)
# update 2025-02-23 04:31:15
# update 2025-05-06 01:36:42
