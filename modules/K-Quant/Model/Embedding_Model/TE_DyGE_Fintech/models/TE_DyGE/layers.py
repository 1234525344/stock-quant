    ...
    def sp_attn_head(self, seq, in_sz, out_sz, adj_mat, activation, in_drop=0.0, coef_drop=0.0, residual=False,
        ...
        with tf.variable_scope('struct_attn', reuse=reuse_scope):
            if sparse_inputs:
                ...
                # weight_var = tf.Print(weight_var,[tf.shape(weight_var)],'weight_var')
                # print('in_sz, out_sz',in_sz, out_sz)
            ...
            # seq_fts = tf.Print(seq_fts,[tf.shape(seq_fts)],'seq_fts_1') # [1 N T] [1 1435 8] [1 1094 8] -> feats shape
            ...
            ### PRINTER in tensorflow ### HOLY  

            # ogits = tf.sparse_add(aaa, bbb)
            #logits = tf.sparse_add(adj_mat * tf.transpose(f_2),adj_mat * f_1)
            ...
            # seq_fts = tf.Print(seq_fts,[tf.shape(seq_fts)],'seq_fts_2')
            seq_fts = tf.squeeze(seq_fts)
            # seq_fts = tf.Print(seq_fts,[tf.shape(seq_fts)],'seq_fts_3')
            # coefficients = tf.Print(coefficients.values,[coefficients.dense_shape],'coefficients') 
