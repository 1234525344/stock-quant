#!/usr/bin/env python
"""Final rebuild: TOC with proper tab stops + dot leaders, all format per image spec."""
import sys, re
...
def title(text):
    return P(text, TITLE_FONT, Pt(22), True, 'C', sb=6, sa=10)

def subhead(text):
    return P(text, TITLE_FONT, Pt(14), True, sb=10, sa=4)

def body(text):
    return P(text, BODY_FONT, Pt(14), indent_pt=28)

def body0(text):
    return P(text, BODY_FONT, Pt(14))

def blank():
    return P('')

def toc_item(p, item_name, page_num):
    """Set TOC paragraph with proper tab stop + dot leader."""
    ...
    tab.set(qn('w:pos'), '8844')  # ~156mm = right edge of text area in twips (156/25.4*1440)
    ...
    if p.runs: p.runs[0].text = f'{item_name}\t{page_num}'
        ...
        r = p.add_run(f'{item_name}\t{page_num}'); mk(r, BODY_FONT, Pt(14))
...
for item_name, page_num in toc:
    p = P(indent_pt=0)  # no indent for TOC items
    toc_item(p, item_name, page_num)
...
subhead('2. 收获与体会')
body('（1）工程绘图与办公技能从生疏到熟练。从一开始对三维建模、二维出图不熟练，到能够独立完成模型修改、工程图纸输出，我真正掌握了工程绘图的规范和行业标准。通过编制BOM表和物料清单，养成了严谨细致、反复核对的工作习惯，明白了工程资料容不得马虎。同时，规范制作工程报表、质检记录表、物料统计表的能力也有了实质性的提升，能够熟练整理归档图纸、工艺文件、检测报告，保证工程资料完整清晰可追溯。')
body('（2）制造工艺认知从课本到实践的跨越。在学校课堂上学习机械制造工艺、金属材料学等课程时，更多停留在书本概念。通过在日常工作中接触铝材、锌合金、不锈钢等金属原材料的实际加工应用，学习压铸、冲压、抛光、电镀、氧化、喷砂等表面处理工艺，我对材料特性和工艺适配性的理解从书本走向了生产现场。通过接触氧化、喷砂等表面处理工艺，以及参与成品装配、品检、镭雕等实操工作，我跳出了单纯的图纸层面，真正了解五金电子产品从设计、打样、表面处理、装配到质检量产的完整流程，懂得了设计要贴合生产实际，工艺标准要贴合客户需求和工厂加工能力。')
body('（3）职场思维方式从学生到职业人的转变。实习让我学会了主动沟通、主动跟进、主动承担。项目跟进过程中需要对接工程师、生产、品检、业务等多个部门，锻炼了协调能力和执行力。协助其他部门工作的过程也让我懂得团队配合的重要性——任何岗位都是公司整体流程中不可或缺的一环，要有大局意识和配合意识。')
...
# ===== SAVE =====
outpath = r'C:/Users/lb/Desktop/岗位实习总结_定稿_v2.docx'
doc.save(outpath)
total = sum(len(p.text.replace(' ','').replace('\n','')) for p in doc.paragraphs)
print(f'Done: {len(doc.paragraphs)}p ~{total}c -> {outpath}')
